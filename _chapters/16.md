...
Toaq words can have "prefixes". These are written with an underdot beneath the first vowel, and pronounced as a little glottal stop during that vowel.
...
These underdots can be hard to type on a phone computer, and sadly, many fonts don't support them. So you'll just as often see people write prefixed words by simply placing a hyphen or colon after the prefix, like _buq-foıge_{:.t} or _ge:chaı_{:.t}.
...
The prefix _bẹ-_{:.t} turns verbs into "generic adjectives": if _toa_{:.t} is _word_, then _bẹtoa_{:.t} means "of words" or _verbal_.
...
Short pronouns have prefix forms that fill the object slot of a verb. The result is a new single-slot verb, and so it can be used as a noun:

<blockquote markdown="1">

_paı_{:.t}<br>▯ is a friend to ▯

_**sụq**paı_{:.t}<br>▯ is a friend to **you**

_ké sụqpaı_{:.t}<br>your friend
...
_**tụ**paı_{:.t}<br>▯ is a friend to **everyone**

_kuı_{:.t}<br>▯ is because of ▯

_hı̣kuı_{:.t}<br>▯ is because of **what**?
...
You can have more than one prefix on a word. The glottal stop and underdot go on the last prefix.
...
To parse a hypothetical word like _beıcoeqdạıfeugımha_{:.t}, we split off _raku_ from the front until we find the underdot:
...
Well, when a Toaq word is written with a vowel at the start, there's actually an implied glottal stop at the start that we don't write. In the middle of a word, we do write this glottal stop as an apostrophe.
