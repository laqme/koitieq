...
In grammar, **aspect** tells us how the time we're **referencing** relates to the time an event **happened**.
...
The default aspect is _tam_{:.t}, which views the event as a whole and places it within the reference time. You can say it explicitly to really emphasize this "point-event" perspective.
...
In semantics land, the reference time is the time interval _t_ contributed by the tense particle, and an event has a **temporal trace** _τ(e)_ telling us the interval it plays out over.
...
We can't really analyze a sentence like _"I was logging in, but then the computer crashed"_ as referring to a complete logging-in event, as no such event exists. We are refering to what _would_ have been a complete logging-in event in an alternative world where the computer didn't crash.

Such alternative worlds, where things that stop us from what we're doing don't happen and the future is maximally compatible with the past, are called **inertia worlds**. They were introduced by David Dowty in his 1979 book _Word Meaning and Montague Grammar_ to explain sentences like the above.

In our case, _pu chum haqbaı jí_{:.t} is analyzed like so:

> For all inertia worlds _w_ branching off from the past time _t_, there was some cooking-event in _w_ such that _t_ ⊆ _τ_(_e_).

This way, we can use the progressive aspect even for events that never quite fully take shape in the world as we're discussing it.
...
A **donkey sentence** is a sentence that contains a variable outside of its scope island.
...
These sentences are hard to account for when translating Toaq sentences into logic formulas, because such occurences are "obviously" disallowed in formal logic.

> ∀[X: _poq_{:.t}(X) ∧ ∃[Y: _aqshe_{:.t}(Y)] _bo_{:.t}(X, Y)] _kıaı_{:.t}(X, <span style="text-decoration:underline wavy #e02;color:#e02">Y</span>)

One perspective on this phenomenon is that natural language is being _informal_ or _inexact_, and we should just analyze this _it_ as somehow proxying for "the donkey she feeds." That explanation obviously doesn't quite suit our purposes.

For a while, Toaqists wondered if speaking a loglang means learning not to produce donkey sentences. Should we just say "every person feeds each donkey she owns"? But donkey pronouns are really everywhere once you start noticing them. Can't we just make them work somehow?

There are some theories of natural language syntax that try to explain the donkey anaphora phenomenon "as-is", and there are in turn some theories of Toaq syntax that try to adopt those explanations. One of these is based on **continuations**: see _Donkey anaphora is in-scope binding_ (chapter 4) by Barker and Shan (2008).
